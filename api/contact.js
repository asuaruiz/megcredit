...
function emailLayout(content, previewText) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${escapeHtml(previewText)}</title></head><body style="margin:0;background:#F1EDE4;font-family:Arial,sans-serif;color:#1A1A1A"><div style="display:none;max-height:0;overflow:hidden">${escapeHtml(previewText)}</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#F1EDE4;padding:28px 12px"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:620px;background:#FFFFFF;border-radius:8px;overflow:hidden;border:1px solid #E9E5DC"><tr><td style="background:#163A5F;padding:30px 36px"><div style="font-family:Georgia,serif;color:#D2AE5C;font-size:24px;font-weight:bold;letter-spacing:6px">MAGIC</div><div style="color:#F8F6F2;font-size:10px;font-weight:bold;letter-spacing:4px;margin-top:5px">ENTERPRISE GROUP</div></td></tr><tr><td style="padding:38px 36px">${content}</td></tr><tr><td style="background:#0E2740;padding:24px 36px;color:#BCC7D4;font-size:12px;line-height:1.7">Magic Enterprise Group · Orlando, Florida<br><a href="https://www.megcredit.com" style="color:#D2AE5C">megcredit.com</a> · <a href="tel:+14077358696" style="color:#D2AE5C">+1 407-735-8696</a><br><span style="color:#8FA0B4">This message relates to a request submitted on megcredit.com.</span></td></tr></table></td></tr></table></body></html>`;
}

function confirmationEmail(submission) {
  return emailLayout(`<p style="margin:0 0 8px;color:#A67C1B;font-size:12px;font-weight:bold;letter-spacing:2px;text-transform:uppercase">Request received</p><h1 style="margin:0 0 18px;font-family:Georgia,serif;color:#163A5F;font-size:30px;line-height:1.2">Thank you, ${escapeHtml(submission.name)}.</h1><p style="margin:0 0 18px;color:#4A4A4A;font-size:16px;line-height:1.7">We received your evaluation request. Our team will review your details and reach out to explain the next step.</p><div style="margin:24px 0;padding:18px 20px;border-left:3px solid #A67C1B;background:#F8F6F2;color:#4A4A4A;font-size:14px;line-height:1.6"><strong style="color:#163A5F">Important:</strong> please don't reply to this email with credit reports, passwords, banking information, or your full Social Security number.</div><p style="margin:22px 0 0;color:#4A4A4A;font-size:14px">If you need immediate help, you can reach us on WhatsApp at <a href="https://wa.me/14077358696" style="color:#163A5F;font-weight:bold">+1 407-735-8696</a>.</p>`, 'We received your evaluation request');
}

function managerEmail(submission) {
  const row = (label, value) => `<tr><td style="padding:9px 12px;border-bottom:1px solid #E9E5DC;color:#7C5C14;font-size:12px;font-weight:bold;text-transform:uppercase;vertical-align:top;width:130px">${label}</td><td style="padding:9px 12px;border-bottom:1px solid #E9E5DC;color:#1A1A1A;font-size:14px;line-height:1.5">${escapeHtml(value || 'Not provided')}</td></tr>`;
  return emailLayout(`<p style="margin:0 0 8px;color:#A67C1B;font-size:12px;font-weight:bold;letter-spacing:2px;text-transform:uppercase">New contact</p><h1 style="margin:0 0 20px;font-family:Georgia,serif;color:#163A5F;font-size:28px">New request from megcredit.com</h1><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border:1px solid #E9E5DC;border-radius:6px;overflow:hidden">${row('Name', submission.name)}${row('Phone', submission.phone)}${row('Email', submission.email)}${row('Goal', submission.goal)}${row('Message', submission.message)}</table><p style="margin:24px 0 0"><a href="mailto:${encodeURIComponent(submission.email)}" style="display:inline-block;background:#163A5F;color:#FFFFFF;text-decoration:none;padding:12px 20px;border-radius:6px;font-size:14px;font-weight:bold">Reply to client</a></p>`, `New request from ${submission.name}`);
}
    ...
    body: JSON.stringify([
      {
        ...
        to: [submission.email],
        reply_to: manager,
        subject: 'We received your evaluation request | MEG Credit',
      ...
      },
      {
        ...
        subject: `New request from ${submission.name} | MEG Credit`,
