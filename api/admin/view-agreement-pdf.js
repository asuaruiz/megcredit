...
export default async function handler(request, response) {
  ...
  try {
    const agreement = await supabaseOne(`megcredit_service_agreements?id=eq.${request.query.id}&select=pdf_storage_path,pdf_original_filename`, { method: 'GET' });
    ...
    console.error('View agreement PDF failed', error instanceof Error ? error.message : 'unknown error');
