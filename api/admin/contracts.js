...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (!agreementsResponse.ok || !accountsResponse.ok) throw new Error('Contract query failed');
    const agreements = await agreementsResponse.json();
    const accounts = await accountsResponse.json();
    return json(response, 200, { contracts: agreements.map(({ pdf_storage_path: path, ...agreement }) => ({
      ...agreement,
